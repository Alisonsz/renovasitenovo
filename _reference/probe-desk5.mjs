...
const r=await p.evaluate(()=>[...document.querySelectorAll('div[class*="overflow-x-auto"]')].map(el=>{const cs=getComputedStyle(el);const sec=el.closest('section');const h=sec?sec.querySelector('h2'):null;const cards=[...el.querySelectorAll('[data-carousel-item]')].length;return {name:h?h.textContent.trim().slice(0,18):'Destaques',snap:cs.scrollSnapType,overflowX:cs.overflowX,cards};}));
